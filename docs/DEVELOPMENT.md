...
일상 개발과 자동 검증에서 분리된 수동 runbook은 아래 문서를 따른다.

| 목적 | 정본 |
| --- | --- |
| 프론트 데모 카탈로그 데이터 적재 | `docs/DEMO_SEED.md` |
| 성능 테스트용 더미 데이터 적재와 k6 실행 | `docs/PERFORMANCE_TESTING.md` |

두 절차 모두 로컬 MySQL과 Redis를 전제로 하며, 일반 `test`와 완료 게이트에는 자동 포함되지 않는다.
