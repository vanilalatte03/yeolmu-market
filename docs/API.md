...
| P0 | 채팅 | 채팅방 저장 실패 보정 구독 | `SUB` | `/sub/chat-rooms/{roomId}/errors` |
...
상품 검색 결과 캐시와 인기 검색어 집계는 `docs/adr/002-redis-search-cache.md`를 따르며, v2 검색 캐시의 목록/표시 캐시 분리와 무효화 정책은 `docs/adr/011-product-search-cache-partial-eviction.md`를 따른다.
...
v2 검색 결과 캐시는 Redis `CacheManager`로 관리하며, TTL은 애플리케이션 설정으로 관리한다. 기본값은 목록 캐시 30초(`yeolmu.cache.search.products-v2.list-ttl`), 표시 캐시 5분(`yeolmu.cache.search.products-v2.display-ttl`)이다.
...
| 이름 | 타입 | 필수 | 설명 |
| --- | --- | --- | --- |
| `beforeMessageId` | Long | N | 이 메시지의 생성 시각보다 이전 메시지 조회. 같은 생성 시각이면 `messageId` 기준 |
...
```json
{
  "messages": [
    {
      "messageId": 500,
      "acceptedMessageId": "5f2a6fd7-5c5b-48de-9d4e-0c4629f50e63",
      "roomId": 300,
      "senderId": 2,
...
메시지 저장은 비동기로 처리되므로 실시간 수신 payload의 `messageId`는 `null`이며, 이전 메시지 조회 API는 같은
`acceptedMessageId`와 저장 후 확정된 `messageId`를 함께 반환한다. `acceptedMessageId`는 접수된 실시간 메시지와 후속 저장 실패
알림을 매칭하기 위한 서버 생성 ID다.
...
구독 destination `/sub/chat-rooms/{roomId}`로 접수된 메시지가 발행된다. 메시지 저장은 비동기로 처리되므로 실시간 발행 payload의
`messageId`는 `null`이며, 이전 메시지 조회 API는 같은 `acceptedMessageId`와 저장 후 확정된 `messageId`를 함께 반환한다.
`acceptedMessageId`는 접수된 실시간 메시지와 후속 저장 실패 알림을 매칭하기 위한 서버 생성 ID다.
...
SEND 요청의 검증 또는 비즈니스 오류는 메시지를 저장·발행하지 않고, `/user/queue/errors`로 아래 payload를 전송하며 연결은 유지한다.
...
발행 이후 비동기 저장 또는 저장 작업 등록에 실패하면 서버는 방 단위 보정 destination `/sub/chat-rooms/{roomId}/errors`로 아래 payload를
후속 전송한다.
발신자에게는 기존 `/user/queue/errors`로도 같은 payload를 전송한다. 이 경우 이미 수신된 실시간 메시지는 저장되지 않았으므로
클라이언트는 `acceptedMessageId`가 같은 pending 메시지를 실패 처리한다. 발신자가 방 보정 destination과 user error queue를 모두
구독하면 같은 `acceptedMessageId`를 중복 수신할 수 있으므로 `acceptedMessageId` 기준으로 멱등 처리한다.
