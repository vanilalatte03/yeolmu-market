...
- **Flyway 도입** — migration 파일과 적용 이력을 관리해야 하지만, 코드 변경과 DB 변경의 순서를 명확히 남길 수 있다.

## 결정

DB 스키마 변경은 Flyway migration으로 관리한다.
기존 MySQL 스키마는 이미 존재한다고 보고 `baseline-on-migrate`로 기준점을 잡은 뒤, 이후 변경분부터 `V*__*.sql` migration으로 적용한다.
...
- 기존 DB에 Flyway 이력이 없으면 애플리케이션 시작 시 baseline 후 신규 migration을 적용한다.
- 빈 DB를 처음부터 완성 스키마로 만드는 `V1__init_schema.sql`은 이번 결정의 범위가 아니며, 필요하면 별도 이슈로 다룬다.
